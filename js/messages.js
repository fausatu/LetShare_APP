...
async function silentRefreshMessagesList() {
    try {
        ...
        var statusPriority = { 'pending': 5, 'accepted': 4, 'partial_confirmed': 3, 'rejected': 2, 'cancelled': 1, 'completed': 1 };
        var conversationMap = {};
        ...
        var visibleConversations = deduplicatedConversations.filter(function(conv) {
            ...
            return status === 'pending' || status === 'accepted' || status === 'partial_confirmed' || status === 'completed' || status === 'rejected' || status === 'cancelled';
        });
...
async function loadMessagesList() {
    ...
    messagesListContainer.innerHTML = '<div style="text-align: center; padding: 2rem; color: #9ca3af;">' + t('loadingConversations') + '</div>';
    ...
    try {
        ...
        var visibleConversations = deduplicatedConversations.filter(function(conv) {
            var status = String(conv.status || 'pending').toLowerCase().trim();
            var isVisible = status === 'pending' || status === 'accepted' || status === 'partial_confirmed' || status === 'completed' || status === 'rejected' || status === 'cancelled';
            if (conv.hidden) return false;
            return isVisible;
        });
        ...
        visibleConversations.forEach(function(conversation, index) {
            ...
            // Completed/Rejected badge
            var convStatus = String(conversation.status || 'pending').toLowerCase().trim();
            var itemStatus = String(conversation.itemStatus || '').toLowerCase().trim();
            var isCompleted = convStatus === 'completed';
            var isRejected = convStatus === 'rejected';
            var isCancelled = convStatus === 'cancelled';
            var isItemTaken = isRejected && (itemStatus === 'accepted' || itemStatus === 'completed' || itemStatus === 'deleted');
            var statusBadge = '';
            if (isCompleted) {
                statusBadge = '<span style="background: #dbeafe; color: #1d4ed8; font-size: 0.65rem; font-weight: 600; padding: 2px 8px; border-radius: 10px; white-space: nowrap; flex-shrink: 0;">' + t('badgeCompleted') + '</span>';
            } else if (isCancelled) {
                statusBadge = '<span style="background: #f3f4f6; color: #6b7280; font-size: 0.65rem; font-weight: 600; padding: 2px 8px; border-radius: 10px; white-space: nowrap; flex-shrink: 0;">' + t('badgeCancelled') + '</span>';
            } else if (isItemTaken) {
                statusBadge = '<span style="background: #fef3c7; color: #92400e; font-size: 0.65rem; font-weight: 600; padding: 2px 8px; border-radius: 10px; white-space: nowrap; flex-shrink: 0;">' + t('itemNoLongerAvailableStatus') + '</span>';
            } else if (isRejected) {
                statusBadge = '<span style="background: #fee2e2; color: #dc2626; font-size: 0.65rem; font-weight: 600; padding: 2px 8px; border-radius: 10px; white-space: nowrap; flex-shrink: 0;">' + t('badgeRejected') + '</span>';
            }
